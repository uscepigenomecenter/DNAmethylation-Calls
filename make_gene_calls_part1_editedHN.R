...
##edited this part after TCGA presentation. Realized we should take 10, 50, 90th from each normal tissue and then average them. Thus I redid all three steps. all new data saved...Jan 13, 2012 12PM
b.q <- apply(brca, 1, quantile, c(.1,.5,.9), na.rm=T)
k.q <- apply(kirc, 1, quantile, c(.1,.5,.9), na.rm=T)
l.q <- apply(lusc, 1, quantile, c(.1,.5,.9), na.rm=T)
b.q <- t(b.q); dimnames(b.q)[[2]] <- c("B10","B50", "B90")
k.q <- t(k.q); dimnames(k.q)[[2]] <- c("K10","K50", "K90")
l.q <- t(l.q); dimnames(l.q)[[2]] <- c("L10","L50", "L90")

nor <- cbind(b.q, k.q, l.q)
nor <- nor[,c(1,4,7,2,5,8,3,6,9)]
nor.10 <- apply(nor[,1:3], 1, mean, na.rm=T)
nor.50 <- apply(nor[,4:6], 1, mean, na.rm=T)
nor.90 <- apply(nor[,7:9], 1, mean, na.rm=T)

nor.per.450 <- cbind(nor.10, nor.50,nor.90)
